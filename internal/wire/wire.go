...
import (
	"bytes"                  // Needed for PracticeReportCmdRunner methods
	"encoding/json"          // Needed for RunSearchSchool & PracticeReportCmdRunner methods
	"fmt"                    // Needed for RunSearchSchool & PracticeReportCmdRunner methods
	"io"                     // Needed for PracticeReportCmdRunner methods
	"log"                    // Needed for runners
	"mime/multipart"         // Needed for PracticeReportCmdRunner methods
	"net/http"               // Needed for PracticeReportCmdRunner methods
	"net/url"                // Needed for PracticeReportCmdRunner methods
	"os"                     // Needed for PracticeReportCmdRunner methods
	"time"                   // Needed for PracticeReportCmdRunner methods
	_ "xixunyunsign/service" // Use blank identifier to satisfy compiler
	"xixunyunsign/service/impl"
	"xixunyunsign/utils"
	"xixunyunsign/web"
	// "xixunyunsign/cmd" // No longer needed for GenerateContent payload types
	"xixunyunsign/service" // Need service for runner dependency

	"github.com/google/wire"
)

// --- Structs moved from cmd/struct.go to break import cycle ---
...
type PracticeReportCmdRunner struct {
	...
	// HttpClient *http.Client // Consider injecting if needed
}
...
// --- Methods for PracticeReportCmdRunner ---
...
func InitializePracticeReportCmdRunner() (*PracticeReportCmdRunner, error) {
	wire.Build(
		RepositorySet,
		// ServiceSet is not directly needed if runner only depends on repo
		NewPracticeReportCmdRunner,
	)
	return nil, nil
}
